...
export default {
  getAllUsers: async (request, response) => {
    ...
    if (!users) {
      return response.status(500).json({
        error: "Une erreur est survenue pour afficher les utilisateurs.",
      });
    }
    return response.status(200).send(users);
  },

  getUsersWithoutRoles: async (__, response) => {
    const users = await searchDatamapper.findUserWithoutRoles();
    if (!users) {
      return response.status(500).json({
        error: "une erreur est survenue pour afficher les utilisateurs.",
      });
  ...
  getUsersByActivity: async (request, response) => {
    ...
    const users = await searchDatamapper.findUsersByCenterByActivity(
      centerId,
      activityId
    );
    if (!users) {
