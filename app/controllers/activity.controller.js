...
export default {
  ...
  addNewActivity: async (request, response) => {
    ...
    const linkActivityToCenter =
      await CenterHasActivityDataMapper.linkCenterWithActivity(
        centerId,
        activityId
      );
    if (!linkActivityToCenter) {
      ...
      return response
        ...
        .json({
          error:
            "Une erreur est survenue lors de la liaison de l'activité au centre",
        });
    }
    await client.query("COMMIT");
    return response
      .status(200)
      .send({ activity: activityAdded, link: linkActivityToCenter });
  },
  ...
  modificationActivity: async (request, response) => {
    ...
    const checkActivity = await ActivityDataMapper.checkIfExistActivity(
      nameActivity
    );
    if (checkActivity[0]) {
      return response.status(401).json({ error: "L'activité a déjà été crée" });
    }

    const updatedActivity = await ActivityDataMapper.update(
      nameActivity,
      activityId
    );
  ...
  findCenterByActivity: async (request, response) => {
    ...
    const listCenterByActivity =
      await CenterHasActivityDataMapper.findCentersByActivityId(activityId);
    if (!listCenterByActivity) {
      return response.status(500).json({ error: "Une erreur est survenue" });
    }
